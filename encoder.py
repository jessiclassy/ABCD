...
class BLSTMEncoder(nn.Module):
    def __init__(self, word_emb_dim, hidden_size, device):
        ...
        logger.info(
            "tied weights = {}, using biredictional cell: {}".format(
                self.tied_weights, bidrectional
            )
        )
    ...
    def forward(self, sent_tuple):
        ...
        len_tmp = torch.tensor([i for i in sent_len]).to(
            self.device
        )  # prevent given numpy array are negative error

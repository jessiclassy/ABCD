    ...
    try:
        for t in dep_lines:
            ...
            if src == "_" or tgt == "_":
                ...
                # print("new tgt", tgt)
                newsrc = (
                    src.split("-")[0] + "-" + re.sub("[^0-9]", "", src.split("-")[1])
                )
                newtgt = (
                    tgt.split("-")[0] + "-" + re.sub("[^0-9]", "", tgt.split("-")[1])
                )
...
locs = [
    ind
    for ind, value in enumerate(deplines)
    if "Dependency Parse (enhanced plus plus dependencies):\n" in value
]
...
for _ in range(4734, 4734 + 1):
    ...
    try:
        ...
        adjs, conn_adjs = BuildAdj(
            vtoi, tmp
        )  # connected nodes by neighbor, adj: nodes connected by dependency arcs
        ...
        golds_out = [
            i.replace("-LRB-", "LRB").replace("-RRB-", "RRB") for i in golds_out
        ]
        s = orig_sents[_].replace("-LRB-", "LRB").replace("-RRB-", "RRB")
        where_it_from, new_temp, cut_temp, del_temp = KEEPORDROP(
            all_pairs, golds_out, itov, s
        )
    ...
    if len(new_temp) == 0:
        ...
        (
            output_arcs[_]["accept"],
            output_arcs[_]["copy"],
            output_arcs[_]["break"],
            output_arcs[_]["drop"],
        ) = (new_temp, copy_temp, cut_temp, del_temp)

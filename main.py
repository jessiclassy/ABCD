...
class SupervisedTrainer(object):
    ...
    def train(self):
        ...
        eval_after = self.config.get(
            "after_eval", 1
        )  # number of epochs starting to eval after
        ...
        for epoch in range(0, self.epoch):
            ...
            for _iter in range(n_iters):
                batched = []
                for _id in permutation[
                    _iter * self.batch_size : (_iter + 1) * self.batch_size
                ]:
                    sample = self.train_db[_id]
                    # Drop sentence with length more than 80 words
                    if len(sample["all_words"]) >= 80:
                        continue
                    sent, length, adj_pair, golds = (
                        sample["sent"],
                        np.array([sample["sent"].shape[0]]),
                        sample["pair_vecs"],
                        sample["gold_labels_tensor"],
                    )
            ...
            print(
                "[TRAINING] EPOCH {} BATCHWISE LOSS : {:.5f}, TOOK TIME {} ".format(
                    epoch, epoch_loss / self.batch_size, end - start
                )
            )
            # iteration done
            ...
            if epoch > eval_after and epoch % eval_every == 0:
                ...
                new_best_score = self.validate_translate(
                    epoch, self.best_score, self.write_csv
                )
    ...
    def validate_translate(self, epoch, best_score, write=False):
        ...
        for _k in permutation:
            ...
            pred_strs = self.model.constructgraph(
                preds, sample["adj_pairs"], sample["adj"], sample["itov"]
            )
            ...
            if self.pred_labels.shape[0] == self.gold_labels.shape[0]:
                self.rec = get_multiclass_recall(
                    self.pred_labels.detach().cpu().numpy(), self.gold_labels, 4
                )
                self.prec = get_multiclass_prec(
                    self.pred_labels.detach().cpu().numpy(), self.gold_labels, 4
                )
        ...
        print(
            "[VALIDATION] EPOCH {} MEAN BLEU SCORES : {:.5f}, MEAN F1 SCORES:  {:.5f},  TOOK TIME {} ".format(
                epoch, np.mean(scores), np.mean(f_scores), end - start
            )
        )
        self.model.train()
...
if __name__ == "__main__":
    ...
    start = time.time()
    # Train Dataloader
    train_data = ComplexSentenceDL(
        cfg["root_dir"] + "data/matchvp/",
        cfg["glove_dir"] + "glove.6B.100d.txt",
        cfg["use_cuda"],
        "Train",
    )
    train_data.Loading()
    # Eval Dataloader
    test_data = ComplexSentenceDL(
        cfg["root_dir"] + "data/",
        cfg["glove_dir"] + "glove.6B.100d.txt",
        cfg["use_cuda"],
        "Test",

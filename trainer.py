...
class Trainer(object):
    ...
    def __init__(
        ...
        self.gat = GAT(2 * hidden_dim, hidden_dim, arc_dim, arc_dim, num_heads).to(
            device
        )
        if cfg["classifer"] == "Bilinear":
            self.classifer = BilinearClassifier(2 * hidden_dim, arc_dim, 4).to(
                device
            )  # num_classes
            ...
            if "multi_layer" in cfg:
                self.classifer = Classifier(
                    2 * 2 * hidden_dim + arc_dim, cfg["multi_layer"]
                ).to(device)
            else:
        ...
        if self.weight_loss:
            self.loss_fn = nn.CrossEntropyLoss(
                weight=torch.tensor(cfg["inverse_label_weights"]).to(device)
            ).to(device)
        else:
    ...
    def main(self, sents, lengths, adj_pairs, golds, mode="Train"):
        ...
        sent_hidden = self.enc(
            (sents.squeeze(0).unsqueeze(1), lengths)
        ).float()  # length x bsize x h

    ...
    for i in pairs:
        ...
        for g in golds:
            if srctk in g and tgttk in g:
                ...
                srctk not in g
                and srctk in " ".join(list(set(golds) - set([g])))
                and tgttk in g
            ):
                cut_temp.append(i)
                used = True
                ...
                tgttk not in g
                and tgttk in " ".join(list(set(golds) - set([g])))
                and srctk in g
            ):
                cut_temp.append(i)
                used = True
...
def KEEPORDROP(all_pairs, golds, itov, s):
    ...
    for i in all_pairs:
        ...
        if set(src_in_golds).intersection(
            set(tgt_in_golds)
        ):  # impossible to be empty set
            accept.append(i)
            used.append(i)
    ...
    for i in lefts:
        src, tgt = i[0], i[1]
        if (
            src == tgt and where_it_from[src] == []
        ):  # self arc, and the node did not appear anywhere
            drops.append(i)
...
def COPY(all_pairs, golds, itov, where_it_from):
    ...
    for i in subj_pairs:
        ...
        if set(src_sent).intersection(set(tgt_sent)) == set(tgt_sent) and set(
            tgt_sent
        ) != set([]):
            copy_pairs.append(i)
        elif set(src_sent).intersection(set(tgt_sent)) == set(src_sent) and set(
            src_sent
        ) != set([]):
            copy_pairs.append(i)
